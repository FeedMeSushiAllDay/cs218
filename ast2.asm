...
;	1. doubleAnswer1 = 400000
	mov dword[doubleAnswer1], 400000 ; immediate value -> memory segment

;	2. doubleAnswer2 = doubleVariable2
	mov eax, dword[doubleVariable2] ; put value into register
	mov dword[doubleAnswer2], eax ; move value from register to variable

;=====MOVZX=====
;	3. quadAnswer3 = byteVariable3 (8 bit to 64 bit)
	movzx rax, byte[byteVariable3] ; put in 64 bit register
	mov qword[quadAnswer3], rax ; move from register to variable
...
;	5. quadAnswer5 = doubleVariable5 (32 bit to 64 bit)
	movsxd rdx, dword[doubleVariable5]
	mov qword[quadAnswer5], rdx
...
;	6. wordAnswer6 = wordVariable6a + wordVariable6b
	mov ax, word[wordVariable6a] ; put first variable in register
	add ax, word[wordVariable6b] ; add register and second variable
	mov word[wordAnswer6], ax ; put register value in answer
...
;	9. doubleAnswer9 = doubleVariable9 - wordVariable9
	movsx eax, word[wordVariable9] ; extending wordVariable9 from 16 to 32 bit
	sub eax, dword[doubleVariable9]
...
;	11. quadAnswer11 = doubleVariable11a x doubleVariable11b
	mov eax, dword[doubleVariable11a] ; put first word variable in 32 bit register
	mul dword[doubleVariable11b] ; multiply register value by second variable
	mov dword[quadAnswer11], eax  ; stores lower part of answer
	mov dword[quadAnswer11+4], edx  ; store upper part of answer, offset by 4 bytes
...
;	17. byteAnswer17 = byteVariable17 / 5
	mov al, byte[byteVariable17] ; put first variable in first register
    cbw ; expand the bit size
    mov bl, 5 ; put other value in second register
    idiv bl
